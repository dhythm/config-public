...
8. `[要確認]` 行のみ推測置換
9. `[判読不能]` 部分のみ推測置換
10. 文単位の可読性チェック（意味が読み取れない文を抽出）
11. 抽出文のみ可読性補正（最小変更）
12. 二次検証（非`[要確認]`/`[判読不能]`部分が未変更であることを確認）
13. 保存前タグ残存チェック（`rg -n '\[(要確認|判読不能)\]' "$TARGET_FILE" || true`）
14. 未解決箇所以外でタグが0件であることを確認（1件でも残っていれば未完了として 8 に戻る）
15. 保存
