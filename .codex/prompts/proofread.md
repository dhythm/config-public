...
3. 改行を見直し、話題・論点ごとに段落を再構成する（1段落2〜5文目安、段落間は空行）。語句変更や要約は行わず、文章を統合して意味段落への集約のみ実施。同一段落内では改行をを削除して統合。
   - 段落の区切りが曖昧な箇所は統合せず保留し、候補を示してユーザーに確認してからまとめる。
4. ケバ取り、誤字・脱字を校正
5. 意味が不明な箇所を文脈から推測して復元
   - 補完候補が複数候補が存在するなど、どうしても判断できない場合は、ユーザーに選択肢＋自由回答の形で質問する
6. ユーザーに確認し、承認を得られた場合のみ、復元後の文章を読みやすく整形する
